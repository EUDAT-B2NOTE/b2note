...
VERSION=`grep  "^\s*\VERSION"     $ABSOLUTE_PATH/../b2note/check_b2note.sh | awk -F\" '{print $2}'`
SUB_VERS=`grep "^\s*\SUB_VERSION" $ABSOLUTE_PATH/../b2note/check_b2note.sh | awk -F\" '{print $2}'`

# build rpm
rpmbuild -ba --define "_version $VERSION" --define "_release $SUB_VERS" nagios-plugins-eudat-b2note.spec
